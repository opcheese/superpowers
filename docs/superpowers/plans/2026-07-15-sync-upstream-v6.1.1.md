...
# Main Branch Sync (interactive / humans)

After `agents` landed, `main` — the **interactive, human-in-the-loop** branch — was still 187 commits behind upstream (old ~v5.1.0 baseline). It was synced to v6.1.1 with the SAME re-apply strategy but the OPPOSITE philosophy: keep the human in the loop rather than remove them.

## Decisions (main)

- **Target:** upstream `v6.1.1` (same as agents).
- **Philosophy:** interactive. Where `agents` replaced human gates with escalation + an automated verification gate, `main` re-applies a **mandatory human-verification gate**.
- **Visual companion:** **KEPT** (unlike agents). main is the branch a human drives, so the browser companion stays.
- **Version:** mirror upstream `6.1.1`.

## Main's fork set (lighter than agents)

`main` carries only: `topic-research`, `end-of-day-report`, `end-of-week-report` skills; pnpm examples; and the human-verification additions from `c708819`. It does **not** have the `escalation` skill, headless brainstorming, always-PR finishing, or auto-worktrees (those are agents-only, from `d1534f4`).

## Tasks (main)

### Task M1: Worktree + clean adds
- Worktree `sync/main-v6.1.1` off `upstream/main`.
- `git checkout main -- skills/{topic-research,end-of-day-report,end-of-week-report}` — main's **interactive** versions of the report skills (ask user for format), not agents' full-report versions. Commit.

### Task M2: pnpm
- Same substitution as agents (test-driven-development, using-git-worktrees, finishing-a-development-branch, root-cause-tracing.md). Commit.

### Task M3: Re-apply the human-verification gate (main's counterpart to agents' automated gate)
- **executing-plans:** add a "Demand Human Verification" step (Step 3) before "Complete Development" (renumbered Step 4); update Overview + Core principle.
- **dispatching-parallel-agents:** add "### 5. Demand Human Verification" after integration + a matching item 5 in the Verification checklist.
- **subagent-driven-development:** add an **end-of-run** human sign-off (flow node `Present completed work to human for verification` between the final review and finishing; a "Human verification at the end" paragraph; a Red-Flags entry). Deliberately NOT a per-task gate — that would fight upstream's tuned "Continuous execution" guidance; the paragraph explicitly reconciles the two.
- Commit.

### Task M4: Merge into `main`, tree-exact
```bash
git checkout main
git merge -s ours --no-commit sync/main-v6.1.1
git read-tree --reset -u sync/main-v6.1.1
git commit            # merge commit, parents [old main, integration]; tree = integration
git diff --stat sync/main-v6.1.1 HEAD   # MUST be empty
```

### Task M5: Adversarial code review
- Dispatch `superpowers:code-reviewer` on `git diff upstream/main..main`. Result: **clean, no defects**; one cosmetic nit (find-polluter.sh still `npm`).

### Task M6: Push + cleanup
- `git push origin main`; remove worktree; delete `sync/main-v6.1.1`.

---

# Post-Sync: additions applied to BOTH branches

These are **branch-neutral disciplines** (extracted from a project `CONVENTIONS.md`), useful to humans and agents alike, so they live on both `agents` and `main`:

- **`documentation-hygiene` skill** — consistent doc frontmatter (status/last_verified/owner/area/audience), empty-field-as-marker, changelog-at-top. Built via writing-skills RED/GREEN.
- **Realistic-environment gate in `verification-before-completion`** — verify in a prod-like environment before declaring review-ready. RED/GREEN validated (baseline declared done on unit tests; with the gate it insists on exercising the real thing).
- **`find-polluter.sh` pnpm fix** — the review nit, closed on both branches.

On `main` these were **cherry-picked** from `agents` (already RED/GREEN-validated there — no re-test needed for identical content).

---

# Execution Notes / Lessons

- **Both syncs used the tree-exact merge** (`merge -s ours` + `read-tree --reset -u`) so each branch's post-merge tree equals the reviewed integration branch byte-for-byte. Verified with an empty `git diff <integration> HEAD`.
- **Side-effect of tree-exact merge:** it removes any file present on the target branch but absent from the integration branch. On `agents` this silently dropped THIS plan doc (it lived on old `agents` but was never copied into the integration branch). Everything else removed was legitimate (upstream's own v6 removals + our intentional companion deletion). **Next time:** either copy fork-only docs into the integration branch before merging, or restore them from the pre-merge tip (`git checkout <old-tip> -- <path>`) afterward — which is how this doc was recovered.
- **New upstream human-in-the-loop touchpoints:** v6.x added human-facing decisions our older fork changes predated (SDD pre-flight/BLOCKED/plan-mandated, TDD exceptions). On `agents` these were redirected to escalation/safe-defaults in a follow-up commit; audit for them on every future sync.
- **Result:** `agents` (autonomous) and `main` (interactive) both on upstream v6.1.1, both reviewed clean, both pushed. See memory `superpowers-fork-sync.md` for the repeatable workflow.
